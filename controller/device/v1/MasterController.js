...
const addMaster = async (req, res) => {
  try {
    ...
    let createdMaster = await dbService.createDocument(Master,dataToCreate);
...
const findAllMaster = async (req,res) => {
  try {
    ...
    let foundMasters = await dbService.getAllDocuments( Master,query,options);
...
const getMasterCount = async (req,res) => {
  try {
    ...
    let countedMaster = await dbService.countDocument(Master,where);
...
const bulkInsertMaster = async (req,res)=>{
  try {
    ...
    let createdMasters = await dbService.bulkInsert(Master,dataToCreate);
    return res.success({ data :createdMasters });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
    }
...
const bulkUpdateMaster = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkUpdate(Master,filter,dataToUpdate);
    if (!result){
      return res.recordNotFound();
    }
...
const partialUpdateMaster = async (req,res) => {
  try {
    ...
    const query = { _id:req.params.id };
    let updatedMaster = await dbService.findOneAndUpdateDocument(Master, query, dataToUpdate,{ new:true });
    if (!updatedMaster) {
      return res.recordNotFound();
    }
    return res.success({ data:updatedMaster });
...
const getMaster = async (req,res) => {
  try {
    ...
    let options = {};
    let foundMaster = await dbService.getSingleDocument(Master,query, options);
    if (!foundMaster){
      return res.recordNotFound();
    }
