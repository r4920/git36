...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'lHq7pOuC6zN6KYq',
      'username':'Dayton.Grimes39',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Dayton.Grimes39' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'z2MJA_JuxQJWsgE',
      'username':'Devyn85',
      'isDeleted':false,
      'isActive':true,
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Devyn85' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
